...
def load_params():
    parameters = {}
    parameters['start_freq'] = np.load(f'./parameters/start_freq.npy')
    parameters['stop_freq'] = np.load(f'./parameters/stop_freq.npy')
    parameters['num_freq_step'] = np.load(f'./parameters/num_freq_step.npy')
    parameters['rbw'] = np.load(f'./parameters/rbw.npy')
    parameters['TxRxPairs'] = np.load(f'./parameters/TxRxPairs.npy')
    parameters['freq'] = np.load(f'./parameters/freq.npy')
    parameters['dist_vec'] = np.load(f'./parameters/dist_vec.npy')
    parameters['AoD_vec'] = np.load(f'./parameters/AoD_vec.npy')
    parameters['AoA_vec'] = np.load(f'./parameters/AoA_vec.npy')
    parameters['doppler_freq'] = np.load(f'./parameters/doppler_freq.npy')
    parameters['range_Nfft'] = np.load(f'./parameters/range_Nfft.npy')
    parameters['angle_Nfft'] = np.load(f'./parameters/angle_Nfft.npy')
    parameters['doppler_Nfft'] = np.load(f'./parameters/doppler_Nfft.npy')
    parameters['x_offset_shift'] = np.load(f'./parameters/x_offset_shift.npy')
    parameters['y_offset_shift'] = np.load(f'./parameters/y_offset_shift.npy')
    parameters['x_ratio'] = np.load(f'./parameters/x_ratio.npy')
    parameters['y_ratio'] = np.load(f'./parameters/y_ratio.npy')
    parameters['ant_loc'] = np.load(f'./parameters/ant_loc.npy')
    parameters['doppler_window_size'] = np.load(f'./parameters/doppler_window_size.npy')
    parameters['cal_arr'] = np.load(f'./parameters/cal_arr.npy')
...
def main():
    ...
    # load parameters
    params = load_params()
    print('waiting for data collections...')
    while(len(os.listdir('./data_queue'))<0):
        continue
    print('Done')
    print('Start Processing & Visualizing...')
    vmin = 0.0#6
    # cal_frame = sorted(os.listdir('./data_queue'))[0]
    cal_frame = params['cal_arr']#np.load(os.path.join('./data_queue',cal_frame))
    ntarget = 3
    enhance_rate = 1
    while(True):
        start = time.time()
