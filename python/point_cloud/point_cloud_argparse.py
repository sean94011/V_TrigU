...
parser.add_argument("-t", "--threshold", help="enter the threshold you want to apply", type=float, default=0.98)
parser.add_argument("-p", "--plot", help="enter True if you want to plot, False if you don't want", type=bool, default=True)
...
def main(plot=args.plot):
    ...
    if plot:
        ...
        point_cloud_plot(axis_value, target_idx, current_scenario, None, all_points=False, ground_truth=ground_truth, plot_data_path=plot_data_path) # All Points
...
def point_cloud_plot(axis_value, target_idx, scenario, point_cloud=None, all_points=False, ground_truth=None, plot_data_path=None):
    ...
    if all_points is not False and point_cloud is not None:
        # Create a colormap to map the normalized values to colors
        colormap = plt.cm.viridis
