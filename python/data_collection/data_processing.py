...
def main():
    ...
    for data in os.listdir(collected_data_folder):
        if data[0] != '1':
            continue
        if data[15:17] != 'cf':
            continue
        ...
        plt.plot(params['dist_vec'],range_profile,label='range_profile')
        plt.xlabel('Range [m]')
        ...
        plt.grid()
        # plt.show()
        plt.savefig(f'./processed_data_plots/1p_with_cf_with_box/{file_name}.jpg')
        plt.close()
        print('Occupancy Detection Frame Duration: ',time.time()-start, '[s]')
        continue
        # Doppler Extraction
        doppler_arr = np.linalg.norm(data_arr,axis=1)
        doppler_arr = np.fft.ifft(doppler_arr,n=params['range_Nfft'],axis=1)
