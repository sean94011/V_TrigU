...
def main():
    # User Setting
    folder_name = '2p_seat_14_with_cf'
    rbw_arr = [10, 300, 800]
    # Radar Setup
    ...
    for rbw in tqdm(rbw_arr):
        ...
        input('Recalibrate? (Press Enter) | Note: Please make sure the FOV is empty. ')
        ...
        np.save('./parameters/cal_arr.npy', calFrame)
        print('Done')
        ...
        folder_name_full = f'{folder_name}_rbw_{rbw}_{datetime.datetime.now().strftime("%m-%d-%Y--%H-%M-%S")}'
        folder_name_full = os.path.join('./collected_data',folder_name_full)
